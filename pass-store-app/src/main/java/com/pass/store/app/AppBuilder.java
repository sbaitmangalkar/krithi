...
import com.pass.store.model.User;
import com.pass.store.model.UserSystemProperties;
import com.pass.store.service.UserServiceProvider;
import com.pass.store.service.UserServiceProviderImpl;

/**
 * Generates the GUI in swing. (Will be updated to JavaFX in next release.)
 * 
...
public class AppBuilder {
	...
	public static final String ABOUT_APP_FILE_NAME = "about-pass-store.txt";
	...
	private String aboutMessage() {
		...
		messageBuilder.append("\n");
		messageBuilder.append(
				"In case of any issues or complaints, you can reach out to him on catch.shyamBaitmangalkar@gmail.com");
		return messageBuilder.toString();
	}

	/**
	 * Creates the app folder and app properties file if not present in the
	 * user's system. Initialization of app parameters are done.
	 * 
	 * @since v1.0
	 */
	private void init() {
		 ...
		 */
		// frameImg = new
		// ImageIcon(getClass().getResource("/resources/images/K_icon.jpg"));
	...
	private void buildUI() {
		...
		register.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				registerUserFrame.setLocation(screenDim.width / 2 - registerUserFrame.getSize().width / 2,
						screenDim.height / 2 - registerUserFrame.getSize().height / 2);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (isUserAlreadyExists) {
					...
					if (existingUserPassword.equalsIgnoreCase(passwordStr)) {
							...
							if (userName.equalsIgnoreCase(userNameStr)) {
								...
								if (allPasswords.size() > 0) {
									for (Map<String, String> userPasswordDetails : allPasswords) {
										for (Map.Entry<String, String> eachWebAndPassword : userPasswordDetails
												.entrySet()) {
						...
						/*
						 * START: Adding ActionListener for Delete Details
						 * Button.
						 */
						deleteDetails.addActionListener(new ActionListener() {
							...
							public void actionPerformed(ActionEvent e) {
								...
								/*
								 * START: Adding ActionListener for Close
								 * Button.
								 */
						...
						refresh.addActionListener(new ActionListener() {
							...
							public void actionPerformed(ActionEvent e) {
								...
								JPanel treePanel = new JPanel(new FlowLayout());
								String displayName = userNameStr.toUpperCase();
								DefaultMutableTreeNode userRoot = new DefaultMutableTreeNode(
										displayName + "'S PASSWORD VAULT");
								List<User> allUsers = service.fetchAllUsersInApp();
								for (User eachUser : allUsers) {
										...
										if (allDetails.size() > 0) {
											for (Map<String, String> userPasswordDetails : allDetails) {
												for (Map.Entry<String, String> eachWebAndPassword : userPasswordDetails
													...
													DefaultMutableTreeNode websiteNode = new DefaultMutableTreeNode(
															webSite);
													websiteNode.add(new DefaultMutableTreeNode(user));
													websiteNode.add(new DefaultMutableTreeNode(password));
							...
							public void actionPerformed(ActionEvent e) {
								...
								registerDetails.addActionListener(new ActionListener() {
									...
									public void actionPerformed(ActionEvent e) {
										...
										List<Map<String, String>> userWebDetails = specificUser
												.getListOfUserPasswords();
								...
								addDetailsFrame.setLocation(screenDim.width / 2 - addDetailsFrame.getSize().width / 2,
										screenDim.height / 2 - addDetailsFrame.getSize().height / 2);
		...
		appFrame.setLocation(screenDim.width / 2 - appFrame.getSize().width / 2,
				screenDim.height / 2 - appFrame.getSize().height / 2);
